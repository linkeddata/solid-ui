...
describe('PeoplePicker.onSelectGroup', () => {
  ...
  it('runs', () => {
    ...
    expect(peoplePicker.onSelectGroup(RdfLib.sym('https://test/#test'))).toEqual(undefined)
  })
})
...
describe('GroupPicker.render', () => {
  ...
  it('runs', () => {
    ...
    const book = RdfLib.sym('https://test/#test')
    const handler = () => { }
...
describe('GroupPicker.loadGroups', () => {
  ...
  it.skip('should error if not okay', () => {
    const container = document.createElement('div')
    const book = RdfLib.sym('https://test/#book')
    const handler = () => { }
    const groupPicker = new GroupPicker(container, book, handler)
    jest.clearAllMocks() // have to clear otherwise not the correct indices below
    const spyOnNowOrWhenFetched = jest.spyOn(fetcher, 'nowOrWhenFetched')
    ...
    groupPicker.loadGroups()
    const whenFetchedCallback: any = spyOnNowOrWhenFetched.mock.calls[0][1]
    whenFetchedCallback(null, undefined)
...
describe('Group.render', () => {
  ...
  it('runs', () => {
    const groupArg = RdfLib.sym('https://test/#test')
    const element = document.createElement('p')
